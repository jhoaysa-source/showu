...
// Importació del context d'autenticació per saber si hi ha un usuari connectat
import { useAuth } from '../context/AuthContext'

// Per enllaçar amb les rutes internes de l'aplicació
import { NavLink } from 'react-router-dom'

// Importació de les icones socials
import iconTw from '../assets/icon_tw.svg'
import iconTk from '../assets/icon_tk.svg'
import iconInsta from '../assets/icon_insta.svg'

function Footer() {
  const { user } = useAuth() // Obté l'usuari actual per mostrar enllaços personalitzats

  return (
    // Contenidor principal del footer amb fons negre i text blanc
    <Box bg="#0C0C0C" color="white" borderTop={'0.5px solid #03DB88'} py={10}>
      <Box maxW="1200px" mx="auto" px={{ base: 4, md: 6 }}>
        {/* Distribució general: columnes en mòbil, files en desktop */}
        <Box
          display={{ base: 'block', md: 'flex' }}
          justifyContent="space-between"
          textAlign={{ base: 'center', md: 'left' }}
        >
          {/* Bloc esquerre: logotip, descripció i xarxes socials */}
          <Box mb={{ base: 10, md: 0 }}>
          ...
          {/* Bloc dret: enllaços útils i recursos */}
          <Box display="flex" gap={12} justifyContent={{ base: 'center', md: 'flex-start' }}>
            {/* Enllaços principals de navegació */}
            <Box>
              ...
              <Stack spacing={3}>
                ...
                {/* Mostra "Crea" o "Login" segons si l'usuari està autenticat */}
                <NavLink to={user ? '/crea' : '/login'}>
                  <Text color="white" _hover={{ color: '#03DB88' }}>Crea</Text>
                </NavLink>
                {/* Mostra "Perfil" o "Login" segons si l'usuari està autenticat */}
